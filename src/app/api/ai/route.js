...
export async function POST(req) {
  try {
    ...
    if (type === 'evaluate_agenda') {
      ...
      prompt = `You are an expert technical interviewer's assistant. Based on the following interview transcript, generate 3 highly relevant and insightful follow-up questions to ask the candidate. Keep them concise and challenging but fair.\n\nCRITICAL RULE 1: If the transcript is only introductory greetings (e.g., "Hello", "Can you hear me?"), or lacks sufficient technical substance to ask meaningful follow-ups, output EXACTLY the phrase "Waiting for candidate to provide more context..." and nothing else.\n\nCRITICAL RULE 2: If generating questions, output ONLY the 3 questions, each on a new line. Do NOT include any introductory text, conversational filler, or ending text. Do NOT include quotes inside the questions (e.g., no "To jump right in..."). Just the raw, direct questions. Start each question with a number.\n\nTranscript:\n${transcript}`;
    }
